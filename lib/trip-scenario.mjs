...
function placeText(point, label) {
  ...
  const seen = new Set();
  const places = [
    ...(point.facility_context?.matches || []),
    ...(point.geo_context?.nearby || []).map(place => ({...place, relation: 'nearby'}))
  ].filter(place => {
    const key = place.name?.normalize('NFKC').toLocaleLowerCase();
  ...
  const census = metrics?.mean_age?.value != null && metrics?.net_income_household?.value != null
    ? `${label} census snapshot: mean age ${metrics.mean_age.value.toFixed(1)} years; mean net household income €${Math.round(metrics.net_income_household.value).toLocaleString('en')} per year.`
    : `${label} census snapshot: no published local figures are available.`;
  const parts = [`${label}: ${area || 'an unclassified part of Madrid'}.`, census];
  if (inside.length) parts.push(`${label} is inside ${inside.join(', ')}.`);
  if (nearby.length) parts.push(`${label} named places within 100 m include ${nearby.join(', ')}.`);
  if (!inside.length && !nearby.length) parts.push(`${label} has no named relevant place mapped within 100 m.`);
  return parts.join(' ');
}
