...
test('provider state is an explanatory trip scenario with no raw coordinates', () => {
  ...
  assert.match(request.state.trip_scenario, /Origin:/);
  assert.match(request.state.trip_scenario, /Destination:/);
  assert.match(request.state.trip_scenario, /Origin census snapshot: mean age 44\.4 years; mean net household income €51,693 per year/);
  assert.match(request.state.trip_scenario, /Destination census snapshot:/);
